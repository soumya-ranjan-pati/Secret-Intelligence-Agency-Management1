...
class editagent(UpdateView):
    ...
    fields = ('Agent_Name','Dob','Contact_Number','Qualification','Address','image')
...
def ViewClerkjobs(request):
    ...
    if qs:
        ...
        return render(request,'clerkjobs.html')
...
def Assign_Agent(request):
    ...
    CaseDetails(agent_id=agent_id,case_id=case_id,case_name=case_name,evidence=evidence,image=image,status=status).save()
...
class UpEvidence(UpdateView):
    ...
    fields = ('agent_id','case_id','case_name','evidence','status','image')
...
def Create_Case(request):
    ...
    evidence  = request.POST.get("evidence")
    image  = request.FILES['image']
    CaseCreation(case_name=case_name,case_details=case_details,doc=doc,evidence=evidence,image=image).save()
